...
function Http() {
	...
	server.listen(3333);
	console.log('本地服务器创建成功!'.green);
	console.log('在浏览器中输入 localhost:3333 即可访问项目页面'.red);
	server.on('request', function(request, response) { // 当有request请求的时候触发处理函数
		// console.log('...');
		// 解析请求的URL
		var url = require('url').parse(request.url);
		var realPath = path.join(process.cwd(), url.pathname);
		 console.log(realPath.red + '     has load'.green);
		// 特殊URL会让服务器在发送响应前先等待
		switch (url.pathname) {
